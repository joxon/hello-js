...
(async function($, forSTCN) {
  ...
  if ($output.length === 0) {
    ...
    $output = $('<div id="dogeOutput">').css({
      height: '200px',
      overflow: 'auto',
      margin: '0 0 10px'
    }).appendTo($dogeBody)
    $(`<div class="global_header_toggle_button">`).text('　START　').click(() => {
      window.superDoge.start()
    }).appendTo($dogeBody)
    $(`<div class="global_header_toggle_button">`).text('　STOP　').click(() => {
      window.superDoge.stop()
    }).appendTo($dogeBody)
  }
  ...
  async function joinGame() {
    ...
    try {
      ...
      var {
        response
      } = await $.post(`${gameUrlPrefix}/GetPlayerInfo/v0001/`, `access_token=${token}`)
      const {
        active_planet,
        level,
        score: _score,
      ...
      if (!active_planet) {
        ...
        } = await $.get(`${gameUrlPrefix}/GetPlanets/v0001/?active_only=1&language=schinese`)
      ...
      } = await $.get(`${gameUrlPrefix}/GetPlanet/v0001/?id=${active_planet}&language=schinese`)
      planet = planets[0]
      log(`Planet: ${planet.state.name}  Level: ${level}  Exp: ${_score}/${next_level_score}  Team: ${clan_info ? clan_info.name : 'None'}`)
      let zones = planet.zones.filter(({
        captured
      }) => !captured)
      let targetZone
      if (forSTCN && clan_info && clan_info.accountid === stcnId) {
        ...
        zones = zones.reverse()
        targetZone = zones.find(({
          difficulty
        }) => difficulty === 3) || zones.find(({
          difficulty
        }) => difficulty === 2) || zones[0]
      }
      ...
      } = await $.post(`${gameUrlPrefix}/JoinZone/v0001/`, `zone_position=${zone_position}&access_token=${token}`)
      if (zone_info) {
  ...
  async function sendScore() {
    ...
    try {
      var {
        response
      } = await $.post(`${gameUrlPrefix}/ReportScore/v0001/`, `access_token=${token}&score=${score}&language=schinese`)
  ...
  function findTarget(_zones, planet) {
    if (planet.state.capture_progress < 0.15) {
      return _zones.filter(({
        difficulty
      }) => difficulty === 3)[0] || _zones.filter(({
        difficulty
      }) => difficulty === 2)[0] || _zones[0]
    }
    let zones = _zones.sort(({
      capture_progress: a
    }, {
      capture_progress: b
    }) => b - a)
    for (let i = 0; i < 3; i++) {
      target = zones.filter(({
        capture_progress,
        top_clans
      }) => (capture_progress < 1 / (i + 1)) && top_clans[i] && top_clans[i].accountid === stcnId)[0]
      if (target) {
        return target
      }
    }
